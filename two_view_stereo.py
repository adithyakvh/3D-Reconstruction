...
def zncc_kernel(src, dst):
    ...
    sigma_W_1 = np.zeros((M, 1, 3))
    sigma_W_2 = np.zeros((N, 1, 3))
    zncc = np.zeros((M, N, 3))
    ...
    for i in range(M):
        for j in range(N):
            zncc[i, j, 0] = np.sum((src[i, :, 0] - W_1_mean[i, 0]) * (dst[j, :, 0] - W_2_mean[j, 0])) \
                            / ((sigma_W_1[i, :, 0] * sigma_W_2[j, :, 0]) + EPS)
            zncc[i, j, 1] = np.sum((src[i, :, 1] - W_1_mean[i, 1]) * (dst[j, :, 1] - W_2_mean[j, 1])) \
                            / ((sigma_W_1[i, :, 1] * sigma_W_2[j, :, 1]) + EPS)
            zncc[i, j, 2] = np.sum((src[i, :, 2] - W_1_mean[i, 2]) * (dst[j, :, 2] - W_2_mean[j, 2])) \
                            / ((sigma_W_1[i, :, 2] * sigma_W_2[j, :, 2]) + EPS)
